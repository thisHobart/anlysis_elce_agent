...
2026-09-03，`vertexai:gemini-3.8-flash` 经 Cherry `prompt_json` 路径在 10 条真实来源开发集上
达到当前资格门槛（逐案完整准确率 80%）。这证明开发链路可运行，但开发集已参与实现调整，不能代替
独立盲测，也尚未证明重复运行的一致性。
...
数量原文必须带功率单位（含 `瓦` 或拉丁 `W`），否则按 `ambiguous_quantity` 隔离，避免把编号当成 1 台/1 MW。
...
比较只看会影响下游结论的内容：事件类型、相关性、规范化实体键、时刻、容量、方向、状态与物理影响；
...
“一致”，照样放行。以下两道闸门直接读原文，因此模型是否动摇都拦得住：

- **拒绝存疑的“无关”**：`irrelevant` 是终局判定，既不产生事件也不进复核区。正文若含功率量值
...
- **时间锚点覆盖**：正文里每个日期/时刻表达都必须被某个事件的开始或结束时间认领。一句话里报道两次
  破纪录（前一次只有日期、后一次有时刻）时，模型常只取精确的那次，被漏掉的事件不会自己出现在复核区。
  锚点数大于认领数即隔离 `ambiguous_multi_event`。该检查只在存在短期事件时生效，且完整日期时刻
  （`2026年8月3日11时06分`）计为一个锚点而非“日期+时刻”两个。

两道闸门在开发集与盲测集共 20 篇上的行为已逐篇验证：多锚点文档只有 1 篇，两个真无关新闻零信号。
...
- 正文时间锚点多于事件认领的时间（`ambiguous_multi_event`）；
- 引文不是标题或正文的真实子串；
- 必填字段缺少原文证据（含 relevance 必须单独给证据）；
- 短期事件缺少精确开始时刻，或时间精度低于 `hour`；
- 数量语义为未知，或数量原文没有功率单位（疑似把编号当数量）；
- 置信度低于门槛；
- 同文事件无法形成不同的稳定身份；
