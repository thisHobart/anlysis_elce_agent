...
|---|---|
| 文档类型 | P2 当前实现专题 |
| 当前版本 | schema/Prompt `1.3.0`；实体解析快照 `2.0.0` |
| 资格状态 | 未通过；以 [P2 进展状态](phase2-status.md)为准 |
| 配套规则 | [实体来源、类别与组合表达](news-entity-rules.md) |

## 当前状态

实体规则已升级为模型 schema/Prompt `1.3.0` 与实体解析快照 `2.0.0`：市场标签和原文区域分开，
泛称资产独立建类，明确编号枚举确定性拆分，规范化资产不再采用多数票。字段含义、兼容策略与
验收方法见 [P2 实体规则](news-entity-rules.md)。下面的旧版本运行结果仅供历史诊断。

已完成“结构化模型抽取 + 确定性校验 + 安全隔离”的代码边界。现有规则抽取器继续作为合成夹具回归基线；
模型抽取器必须由调用方显式注入，不会因为本次修改而自动访问外部模型。

2026-09-03，`vertexai:gemini-3.8-flash` 经 Cherry `prompt_json` 路径在 10 条真实来源开发集上完成过运行，逐案完整准确率为 80%。2026-09-06 复核确认旧准入规则过宽：完整字段准确率和证据完整性没有阻止 `qualified`。当前 schema/Prompt 为 `1.3.0`；同日开发集重跑仍为 80%，保留集因当前端点协议冲突未完成。当前资格状态统一见 [P2 进展状态](phase2-status.md)。
...
### 实体规范化与来源（schema 1.3.0）

`entities.py` 继续提供旧记录的兼容键；`entity_resolution.py` 为新记录保存带版本的解析快照：

- 标题/正文提及与市场标签各有独立来源，市场标签不再挂靠原文引文；
- 区域受控别名归一，泛称资产进入 `asset_groups`，具体资产单独生成键；
- 只有带父电厂名称的明确编号枚举才拆分，规范化资产共同回链原始组合提及；
- 事件合并、逐时状态、市场匹配、容量特征与基准评估使用解析后的语义；
- 类别键参与身份，缺少具体资产和开始时刻的事件按来源谱系保守去重；
- 原词/规范化名称/来源记录必须一致，来源检查同时验证原文切片、标签位置和拆分规则。

详细字段与验收边界见 [P2 实体规则](news-entity-rules.md)。
...
这些规则已经进入确定性回归；真实模型仍需在兼容的端点配置下用 schema `1.3.0` 重跑保留集，并在开发集与保留集上同时满足严格门槛。
