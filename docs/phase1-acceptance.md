...
当前全量自动化回归覆盖持久化恢复、审批真实性、非法 action、异常终态、单次执行快照、跨修订结果复用、A→B→A 收敛、修订议程重建、混合维度分段比较、Iteration 上限、Episode 隔离、旧 schema 隔离、事件有界化、用户目录、真实函数时间场景，以及 30 函数全量执行、结构诊断逐项断言、叙述层翻译、双 Skill 发现、Markdown/SVG 报告、研究过程可见性、分层研究包、方法与证据映射、三类数据文件直入和重启恢复。发布验证还覆盖受限结果合法停止、结果追问断连后恢复原门禁、offscreen 超时清理，以及当前 EXE 的真实启动和资源加载。
...
1. 研究过程卡在执行期间实时增长，结束后折叠，重启后可展开回看。
2. `report.md` 在桌面内置阅读器中排版正常，SVG 图表随窗口缩放且可打开外部文件。
3. 单文件 EXE 在可见窗口中完成一次含平稳性与朴素基线的真实数据分析，并人工观察关闭重启后的交互状态。
