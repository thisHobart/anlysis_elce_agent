...
| 编写日期 | 2026-08-28（末次更新 2026-09-06） |
| 当前目标 | 合成闭环、两套真实来源语料和模型抽取代码边界已完成；下一步是让真实模型通过严格门槛并取得外部 API 契约 |
...
| 2. 明显新闻事件抽取 | 合成夹具通过；真实规则基线不合格；真实模型严格门槛未通过 | 规则基线召回率为 0%；schema/Prompt `1.3.0` 开发集运行逐案完整准确率为 80%；保留集与重复运行机制已建立，但本轮保留集调用被端点协议冲突阻断 |
| 3. 去重、修订与 `as_of` | 已完成模拟数据实现 | `N01`/`N02`/`N03`/`N10` 归为同一事件且不重复计数；版本库保留全部历史，按 `as_of` 回放时更正生效而公告时间不漂移；完全相同的重复采集被丢弃 |
| 4. 市场时钟与时间对齐 | 已完成模拟数据实现 | 测试使用统一 30 分钟结算网格，代码支持可配置且整除一天的间隔；公告轴与生效轴分别声明零点；提前量可正可负；特征按公告可用时间逐区间门禁，历史快照不产生 `as_of` 之后的行 |
...
- 模型只提出 `ModelNewsExtraction` / `ModelEventCandidate` 候选，本地程序负责原文证据定位，并核对模型数值与原文数字/单位、模型时间与原文日期/钟点、资产与对应引文；通过后才执行单位换算、价格方向映射和最终 `EventRecord` 构造；
- 模型输出不合法、引文不能支持字段值、短期时间不精确、量值含义未知或置信度低于阈值时，采用“校验失败即拒绝进入分析”（fail closed）的策略。一篇新闻中的候选逐个校验：失败候选进入候选级隔离，其他有效事件保留；全部候选失败时才形成文档级隔离；
...
| `version` / `document_version_id` | 来源版本号及由文档身份、内容哈希、版本、发布/更新时间派生的不可变版本 ID；采集时间不参与内容版本身份，同一内容稍后重复抓取不会伪造新版本 |
...
下表的“当前状态”列描述截至 2026-09-06 的代码实现。未实现字段是有意裁剪，不是遗漏。
...
| `document_refs` | 形成该事件判断的文档版本、内容哈希和来源层级列表 | 已实现于 `MergedEvent`；字段优先采用运营方、监管机构、政府等第一方来源，二手报道只保留其更早公开信息的时间和补充证据 |
...
| `cleared_fields` | 更正明确撤回旧值时列出字段；没有再次提及不等于撤回 | 已实现；合并层不会用旧版本值填回明确撤回的字段 |
| `state_history` | 事件每次更正在什么时间变得可用 | 已实现于 `MergedEvent`；历史特征逐区间选择当时可见状态，后来的更正不能改写更正前的值 |
...
抽不出合法事件的文档不会中断整批，也不会退化成伪造字段的事件，而是产生隔离记录。不同市场或缺少市场标签的新闻也逐篇进入 `market_mismatch` 隔离，不再使整批研究失败。除时间、多事件和契约错误外，模型路径还区分模型不可用、响应 schema 无效、原文证据无效、数量语义含糊和低置信度。
...
**已实现。** 无活跃事件时容量影响为确定的 `0`；存在活动事件但至少一个事件容量未知时，总容量保持未知，不能把未知写成零，也不能把部分已知容量冒充总量。构造时对每个结算区间选择当时已经可见的事件修订：停运在 10:00 生效、公告 10:07 可用，则 10:00—10:30 区间计数为 0，10:30 起才计入；12:00 发布的容量更正不会改写 12:00 之前的行。已取消事件不计入，恢复事件在能够可靠匹配同一资产时结束开放的停运状态。

导出给 P1 的扁平表里 `available_at` 恒等于 `timestamp`，因为门禁已经在 P2 侧完成；快照只输出不晚于 `as_of` 的区间，且每行只使用该时点已可获得的事件状态。即使下游忽略该列也无法通过这张表看到未来。集成测试用 P1 的真实加载器读回该表来验证这一点。
...
- 无关新闻正确率 ≥ 95%；
- `unknown` 比率 ≤ 20%；
- 逐案完整准确率 = 100%（处置、类型、相关性、区域、资产、容量和开始时间均正确）；
- 接受事件的证据完整率 = 100%；模型路径对无证据字段直接隔离；
- 模型/网关不可用和 schema 无效属于运行失败，不能冒充“正确隔离”获得隔离召回分。
...
| P2-5 模型资格测试 | 真实模型运行记录、差异分析、首轮质量基线 | P2-3 | 1—2 人日 | 进行中：schema `1.3.0` 开发集严格门槛运行未通过；保留集需解决端点协议冲突后重跑 |

核心测试概念验证约 10—15 人日，另预留约 15% 处理夹具和边界规则迭代。Graph/桌面集成、生产 API 适配器、保留集第二人复核和扩充不包含在该估算中；拿到外部 API 样例和产品出口后再拆分。

上表的“已完成”覆盖合成夹具及模型抽取代码边界。P2-5 的最终资格仍依赖模型选型、真实模型运行和独立盲测集，因此不在本轮结论范围内。

### 9.1 阶段门禁

门禁状态截至 2026-09-06；合成分析和代码边界通过自动化测试，规则基线在真实来源开发集上不合格。真实模型 80% 的旧结果不再满足修正后的完整字段与证据门槛，等待重新测试。
...
- schema/Prompt `1.3.0` 的开发集运行逐案完整准确率仍为 80%；保留集运行被当前端点协议冲突阻断。严格门槛要求完整字段和证据同时达标，故当前不标记为 `qualified`。
...
仍未通过生产门禁的部分：真实模型保留集与重复性、外部 API 适配器，以及用于人工复核排序的持久化置信度。
