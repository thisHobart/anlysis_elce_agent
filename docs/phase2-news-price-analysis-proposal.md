...
| 2. 明显新闻事件抽取 | 合成夹具通过；真实规则基线不合格；真实模型新门槛待重跑 | 规则基线召回率为 0%；Gemini 经 Cherry `prompt_json` 的旧开发集运行中，事件召回、隔离召回和无关新闻正确率均为 100%，但逐案完整准确率只有 80%；独立盲测集与重复运行机制已建立，但必须用 schema 1.2.0 和严格门槛重新运行 |
...
| P2-5 模型资格测试 | 真实模型运行记录、差异分析、首轮质量基线 | P2-3 | 1—2 人日 | 进行中：旧运行只通过宽松门槛；schema 1.2.0 与严格门槛下的开发集、盲测集和重复运行尚待执行 |
...
- Gemini 经 Cherry `prompt_json` 的旧开发集运行逐案完整准确率为 80%；由于新门槛要求完整字段和证据同时达标，该结果不再标记为 `qualified`。
