...
> 状态：已实现（UI 层 + 物化固定）；数据库取数层仍待落地，见 §11
...
UI 层按本规格全部落地：术语红线（§1）、`DataSummary` 与词表（§2）、`DataPanel` 四态（§3–§5）、
交互与失效规则（§6）、合并确认卡片与取数细节弹窗（§7）、会话字段与旧会话兼容（§8）。

§7.2 对后端提的唯一新增要求已完成：两个内置 Skill 的 `references/research-protocol.yaml`
为每个步骤补了 `function_display_text`，编译器把它随方案一起固定下来，确认卡片逐条照读。
协议版本号没有跟着动——证据阶梯、函数集合、判读规则和停止边界都没变，只多了一份给业务人员看的措辞；
改版本号会让已批准但还没执行的方案在生成方法说明时被判为版本不一致。

### 11.1 数据固定（§2.2 物化层）

`app/research/data/sources/materialize.py` 把一次取数写成一份不可变副本：

- **什么时候写**：生成方案时。用户看到确认卡片之前，这批数据已经落盘，所以「他确认的」和「跑的」
  必然是同一批行。
- **写什么**：每条序列归一化后的时间戳与数值（Parquet），加上加载阶段的计数证据、输入清单和
  `DataSummary`。对齐与质量报告不入盘——它们是这些行的纯函数，还原后逐字节一致。
- **放在哪**：`应用数据目录/research/snapshots/<数据指纹>/`。按指纹寻址，因此同一批数据写两次是空操作，
  两批不同的数据也不可能落进同一个目录。默认保留最近 8 份。
- **怎么读**：执行前先按方案锁定的数据指纹还原；还原成功就完全不碰源文件。
- **副本没了怎么办**：回到原来的行为——重新读源文件并比对指纹，数据变了就拒绝执行。
  少一份副本只是多一次读，不会读错数据。

`fetched_at_text` 现在来自物化那一刻，不再每次提问重新盖章。点「取最新的」而数据没变时，
面板保持原时间——同一批数据不会因为又看了一眼就变新；「我刚看过」这件事写在研究过程里。

### 11.2 验收状态

§9 十条全部有自动化测试覆盖：`tests/desktop/test_data_panel.py`、
`tests/research/test_materialization.py`、`tests/research/test_skills_tools.py`、
`tests/research/test_eda_agent.py`。

第 4 条的行为随之改变，且是有意为之：批准后源数据再变，执行**不再中止**，而是照旧跑在固定下来的
那批数据上，结论逐字节不变。只有在副本缺失时才回到「拒绝执行」。

### 11.3 唯一没有逐字照抄的一处

§4.3「进行中项」给的示例是 `已找到风电、光伏、统调负荷，还在看气温`。前半句已实现；
「还在看气温」这半句实现不了：§2.1 的 `DataSummary` 没有字段说明「当前正在看哪一项」，
§2.2 又要求勘探中途的部分摘要用同一结构、未确定的字段留空。两条合起来就没有地方放这个信息。

现在的做法是：提问那一刻就用研究配置里已知的部分点亮面板——电价名称与影响因素清单落为已确定，
时间范围留 `待定`（空心圆），采样间隔作为副行先显示。数据读完后整张卡片补齐。
这满足了 §9.2 的「逐项点亮」，代价是「进行中项」只说找到了什么，不说正在看什么。

要补上后半句，得给 `DataSummary` 加一个字段并改 §2.1，这是规格层面的决定，没有自行改。

### 11.4 旧界面已全部移除

`FileSlotRow`、`InputFilesPanel`、拖拽处理，以及 `fileSlot` / `fileName` / `fileRole` 三条样式，
按 §3 删除干净。随后又清掉三处残留：旧面板提示行的 `contextHint` 样式、两个界面已经够不着的
`set_input_file` / `clear_input_file`，以及合并确认之前的旧方案卡 `PlanMessageWidget`。

由此带来一处能力变化，需要知情：**再也不能单独清空某一路影响因素数据**。旧面板每个槽位有清除按钮，
现在「换一批数据」只能重新指定文件，取消某一路的对话框等于保持原样。这与 §3 删除文件槽是一致的——
选数据是 Agent 的事——但确实少了一个操作。

老会话里 `plan` 类型的方案卡现在由 `DataPlanMessageWidget` 渲染，数据三行显示 `待定`：
那次对话没有存下数据摘要，与其拿今天的数据冒充，不如直说不知道。

### 11.5 仍未落地

数据仍来自本地文件。`DatasetSpec`、`ConnectionProfile` 和数据库取数本身尚未实现，
§10 三条待确认事项保持原样。物化层与取数方式无关：数据库取数接上来之后，
只要产出同样的序列，本节的固定语义不需要再改。
