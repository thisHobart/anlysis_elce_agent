...
|---|---|
| 文档类型 | P1 当前功能说明 |
| 产品状态 | 当前桌面产品能力；发布资格另见 [P1 验收记录](phase1-acceptance.md) |
| 配套文档 | [项目文档导航](README.md)、[系统技术方案](agent-technical-proposal.md)、[持久化研究循环](research-loop.md) |
...
2. **中间连续对话**：用户消息、助手回复、可展开的研究过程、只读方案卡、明确审批操作和结果卡处于同一时间线。
...
叙述遵循同一套审计日志文风：动词开头、句尾不加标点、不使用第二人称；进行中的分析写作 `分析中 · <函数名>` 并附该函数的注册技术说明，完成后写作 `已完成 · <函数名>` 并只附耗时；批量锁定按研究阶段报告构成（例如“数据可用性核验 1 项、电价自身规律 2 项……共 8 项”）。同一函数的“准备执行 / 执行完成 / 结果校验通过”三条事件在界面上折叠为两行，完整三条仍保留在会话文件与研究包中。
...
<output_directory>/agent-{timestamp}-{fingerprint}/
  report.md              # 主报告：由桌面端内置阅读器展示
  methods.md             # 研究设计、逐函数方法、判读阈值、有效性核验与假设验收
  manifest.json
  figures/*.svg
  evidence/
    data_quality.json
...
`report.md` 是用户和评审看的主入口，由桌面端内置阅读器连同 `figures/*.svg` 展示，只渲染本轮真实产生的证据：先给结论与适用边界，再按分析步骤逐节呈现“图 → 读图 → 关键数字”，读图文字由确定性模板结合共享判读阈值生成。方法定义、参数、阈值、假设验收和有效性核验不进入报告正文，集中在 `methods.md`；`methods.md` 由函数目录、锁定计划、领域协议、共享判据、执行轨迹和确定性评估结果共同生成，每个方法都给出结构化证据位置并回链到报告中对应的小节。同一事实只在一处叙述：数据风险只出现在报告的“结论的适用边界”，假设验收只出现在 `methods.md`，桌面结果卡片只保留结论摘要与前三条判断。`provenance/research_plan.json` 保存模型、Skill、协议、函数、变量和参数；`provenance/study_context.json` 保存程序从数据文件推断出的字段与时间轴上下文；`manifest.json` 递归记录全部输入输出 SHA-256、Git 状态和运行环境。相同数据、代码和锁定计划可以重新执行并核对结构化证据。
...
- 研究包包含 Markdown 报告、SVG 图表、对话、计划、执行轨迹、评价和有效哈希。
